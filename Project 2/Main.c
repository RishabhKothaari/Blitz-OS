...
-----------------------------  SimpleThreadExample  ---------------------------------
  ...
  function SimpleThreadExample ()
    ...
    --
      ...
      aThread.Fork (SimpleThreadFunction, 2)  -- Pass "4" as argument to the thread
      SimpleThreadFunction (3)                -- The main thread will loop 7 times
    endFunction
...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    producerMutex: Mutex = new Mutex
    consumerMutex: Mutex = new Mutex
    fillCount:Semaphore =  new Semaphore
    emptyCount:Semaphore = new Semaphore
    bufferMutex:Semaphore = new Semaphore
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  class ForkMonitor
    ...
    fields
      ...
      monMutex:Mutex
      conditionStates: array [5] of Condition
    methods
      Init ()
      PickupForks (p: int)
      PutDownForks (p: int)
      PrintAllStatus ()
      TestPhilosophers(p:int)
  endClass

  -- This method is called just once at the begining of the program to
  -- initialize the condition variables and set the status of Philosophers
  -- to THINKING
  ...
  behavior ForkMonitor
    ...
    method Init ()
      ...
      monMutex.Init()
        ...
        conditionStates = new array of Condition {5 of new Condition}
        for i = 0 to 4
         conditionStates[i].Init()
        endFor
      endMethod
    ...
    method PickupForks (p: int)
      ...
      if !monMutex.IsHeldByCurrentThread()
        monMutex.Lock()
      endIf
        status[p] = HUNGRY
        self.PrintAllStatus()
        self.TestPhilosophers(p)
        while status[p] != EATING
         conditionStates[p].Wait(&monMutex)
        endWhile
        self.PrintAllStatus()
      if monMutex.IsHeldByCurrentThread()
        monMutex.Unlock()
    ...
    -- This method is always called by a thread who is holding Monitor's Mutex.
    -- We check if the thread modelling to check if the philosopher pointed by p
    -- is having both the forks to begin eating.The Signal here wakes up sleeping
    -- thread on respective condition variable.

    method TestPhilosophers (p:int)
         if status[p] == HUNGRY && status[(p-1)%5]!= EATING && status[(p+1)%5]!= EATING
          status[p] = EATING
          self.PrintAllStatus()
          conditionStates[p].Signal(&monMutex)
         endIf
      endMethod
